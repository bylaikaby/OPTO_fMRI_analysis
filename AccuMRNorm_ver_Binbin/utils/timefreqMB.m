...
%             p = 2*pi;
%             s = cycles(index)/5;
%             wav = exp(j*t*p)/sqrt(2*pi) .* ...
%                 (exp(-t.^2/(2*s^2))-sqrt(2)*exp(-t.^2/(s^2)-p^2*s^2/4));
%         
        
%                tmpall(freqind,:)=fftconv2(datapad,wav);
%                 sizewav = size(wav,1)-1;
% r = floor(size(wav,1)/2);padf = zeros(size(datapad));
% padf(1:r+1) = wav(r+1:end);

% padf(end-r+1:end) = wav(1:r);
% freqnew=freqs/srate*(size(datafft,1)+1);deltafreq=floor(min(diff(freqnew)));
% freqnew2=freqnew(1):deltafreq:freqnew(end);
% lfreq=length(freqnew2);
% fftwav=fft(wav,size(datapad,1)).*exp(-1i*2*pi*r*(0:size(datapad,1)-1)/size(datapad,1))';
% ffttotwav=reshape([fftwav*ones(1,lfreq);zeros(deltafreq,lfreq)],[],1);
% ffttotwav(((size(fftwav,1)*lfreq)+1):end)=[];
% ffttotwav=reshape(ffttotwav,[],lfreq);


%convres=ifft((datafft*ones(1,lfreq)).*(ffttotwav));
convres=ifft((repmat(datafft,[1,length(freqs),1])).*(repmat(wavtf,[1,1,size(datafft,3)])));


%freqs=freqnew2*srate/(size(datafft,1)+1);
...
%               mnX=sum(tmpX,1)/size(tmpX,1);
%                 tmpX=bsxfun(@minus,tmpX,mnX);
%                 if strcmpi(g.detrend, 'on'),
%                     for trial = 1:trials
