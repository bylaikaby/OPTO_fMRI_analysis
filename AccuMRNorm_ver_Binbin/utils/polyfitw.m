...
%POLYFIT Fit polynomial to data (supports weights).
%   P = POLYFITW(X,Y,N,W) finds the coefficients of a polynomial P(X) of
%   degree N that fits the data Y best in a least-squares sense. P is a
%   row vector of length N+1 containing the polynomial coefficients in
%   descending powers, P(1)*X^N + P(2)*X^(N-1) +...+ P(N)*X + P(N+1).
%
%   [P,S] = POLYFITW(X,Y,N,W) returns the polynomial coefficients P and a
...
%
%   [P,S,MU] = POLYFITW(X,Y,N,W) finds the coefficients of a polynomial in
...
%      float: double, single
%
%   See also POLY, POLYVAL, ROOTS, LSCOV, POLYFIT

%   Copyright 1984-2011 The MathWorks, Inc.
%   $Revision: 5.17.4.14 $  $Date: 2011/05/17 02:32:30 $
%
%   28.05.14 YM@MPI  weighted version of MATLAB's polyfit().
