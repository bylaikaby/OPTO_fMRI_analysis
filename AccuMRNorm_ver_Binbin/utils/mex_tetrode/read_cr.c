...
versions: Original    05/02    Thanos Siapas
          ...
          Update[?]   03/04/05 Yusuke Murayama@AGLOGO-MPI [supports >2G file]
...
void Get_File_Info( Parameters *parms )
{
  ...
  // to support >2G data file on windows 
  //parms->nmax = (fpstat.st_size - parms->headersize)/parms->cr_rec_size;
  nmax = fpstat.st_size - (unsigned long)parms->headersize;
  nmax = nmax / (unsigned long)parms->cr_rec_size;
  parms->nmax = (long)nmax;

  if( parms->verbose ) {
    //mexPrintf("st_size=%lu header=%ld\n",fpstat.st_size,parms->headersize);
