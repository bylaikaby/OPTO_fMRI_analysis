...
if nargin < 2,  ExcludeRange = [];  end

if iscell(Sig),
  fprintf('\n');
  for N = 1:length(Sig),
    fprintf('%d.', N);
    oSig{N} = matsrmresp(Sig{N},ExcludeRange);
  end
  fprintf('\n');
  return;
end
...
if isempty(ExcludeRange),
  if Nyq < 0.43
    % aliased..
    tmpdf = 1/(60/24.1) - Nyq;  % ~24.1 strokes/min
    ExcludeRange = [-0.025 0.025] + (Nyq - tmpdf);
    %ExcludeRange = [0.10 0.14];
  else
    % no aliasing 
    ExcludeRange = [0.38 0.43];  % ARTHUR default
  end
end;
...
if Nyq < 0.43
  ...
  % no aliasing
  spectrumScaleFactors = [2 1 1]; % ARTHUR default
end
...
for VoxNo = 1:size(Sig.dat,2),
  ...
  %[a,freqaxis] = psd(y,[],Fs,100);  % ARTHUR default, psd(X,NFFT,Fs,WINDOW)

  % pwelch(X,WINDOW,NOVERLAP,NFFT,Fs)
  % [a,freqaxis] = pwelch(y,[],[],[],Fs);
  [a,freqaxis] = pwelch(y,[],[],128,Fs);
