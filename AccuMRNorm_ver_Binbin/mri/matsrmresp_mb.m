function [oSig] = matsrmresp_mb(Sig, ExcludeRange)
%MATSRMRESP_MB - Remove respiratory artifacts by projecting out sinusoids
...
if nargin < 2,
  ExcludeRange = [0.10 0.13];
end;
if iscell(Sig),
  % fprintf('\n');
  for N = 1:length(Sig),
    % fprintf('%d.', N);
    Sig{N} = matsrmresp_mb(Sig{N},ExcludeRange);
  end
  % fprintf('\n');
...
breathe_est_noNorm = zeros(N,1); %contains breathing sinusoids

oSig = Sig;
NFFT=max(256,2^ceil(log2(ceil(size(Sig.dat,2)/8))));
[cs,fax]=multi_pwelch(Sig.dat',NFFT,ceil(size(Sig.dat,2)/8));
cs=cs';
fax=fax*2*pi;
...
%       end
        ...
        breathe_est_noNorm = imag(exp(1i*w_array(whichHarmonic)*n)*exp(1i*phaseArray));
        %breathe_est_noNorm = sin(w_array(whichHarmonic)*n+phaseArray);
        %note: d below is multiplied directly by vector sin(w_array(whichHarmonic)*n+phi_est) to get breathing signal
        dArray(:,whichHarmonic) = y_deflated'*(breathe_est_noNorm./repmat(dot(breathe_est_noNorm,breathe_est_noNorm),size(breathe_est_noNorm,1),1));
      ...
      y_deflated = y_deflated- 1/(breathe_atOptim'*breathe_atOptim)*breathe_atOptim*(breathe_atOptim' * y_deflated);
