...
% YM 12.09.13 improved speed, x1.86
...
%unsed: breathe_est_noNorm = zeros(N); %contains breathing sinusoids
phiEstArray = zeros(1,numHarmonics);
...
for VoxNo = 1:size(Sig.dat,2),
  ...
  if mean(a(a_ind(1:2))) > medianPsd + spectrumScaleFactors(1)*iqrPsd
    ...
    for whichHarmonic = 1:numHarmonics,
      if whichHarmonic >= 2 && mean(a(a_ind(whichHarmonic*2-1:whichHarmonic*2))) <...
      ...
      for k=1:length(phaseArray)
        ...
        %unused: btmp(:,k)=breathe_est_noNorm; 
      ...
      %unused: dEstArray(whichHarmonic) = currentDArray(bestIndex);
      ...
      % 12.09.13 YM : there may be a tiny difference (~1.0e-13), but x1.86 faster...
      %tmpv = 1.0/(breathe_atOptim'*breathe_atOptim);
      tmpv = 1.0/sum(breathe_atOptim.*breathe_atOptim);
      y_deflated = y_deflated - breathe_atOptim*(tmpv*(breathe_atOptim'*y_deflated));
