...
%    0.97 18.03.20 YM@MPI  avoid warning MATLAB:dispatcher:UnresolvedFunctionHandle.
%    0.98 08.04.20 YM@MPI  supports "YDir".
%
...
uicontrol(...
    'Parent',hMain,'Style','Checkbox',...
    'Units','char','Position',[IMGXOFS+IMGW-25 IMGYOFS-4.7 25 1.5],...
    'Tag','YReverseCheck','Value',1,...
    'Callback','mreg2d_gui(''Main_Callback'',gcbo,''y-reverse'',[])',...
    'String','Y-Reverse','FontWeight','bold',...
    'TooltipString','Y-Reverse or not','BackgroundColor',get(hMain,'Color'));
uicontrol(...
    'Parent',hMain,'Style','Checkbox',...
...
switch lower(eventdata)
 ...
 case {'load-ref'}
  ...
  if exist(tfile,'file')
    ...
    set(wgts.StatusField,'String',StatusTxt');  drawnow;
    % avoid warnings in newer MATLABs, when loading data with .regimg(N).tform.tform
 ...
 case {'y-reverse'}
  if get(wgts.YReverseCheck,'value')
    set(wgts.RefAxs,    'YDir','reverse');
    set(wgts.SrcAxs,    'YDir','reverse');
    set(wgts.OverlayAxs,'YDir','reverse');
  else
    set(wgts.RefAxs,    'YDir','normal');
    set(wgts.SrcAxs,    'YDir','normal');
    set(wgts.OverlayAxs,'YDir','normal');
  end
...
IMG.ds  = abs(double(V.mat([1 6 11])));  % ignores any orientation/flip
