...
DATES:
    original 9/91
    program update 6/96
    compile in windows  02/05 Yusuke Murayama@AGLOGO-MPI
...
{
  strcpy(__name->sysname,"windows");
  strcpy(__name->nodename,"");
  strcpy(__name->release,"");
  strcpy(__name->version,"");
  strcpy(__name->machine,"");
#ifdef _GNU_SOURCE
  strcpy(__name->domainname,"");
#endif
  return 0;
}
...
//  * VC++6.0 doesn't support UINT64->double conversion.
//  * double has a precision of 15 digits at least.
double TimestampToDouble(UINT64 timestamp)
{
  double v;
#ifdef _WINDOWS_
  if (timestamp > 0x7fffffffffffffff) {
    timestamp = timestamp - 0x7fffffffffffffff;
    v = (double)((INT64) timestamp) + (double) 0x7fffffffffffffff;
  } else {
    v = (double)((INT64) timestamp);
  }
  v = v / 1000.0;
#else
  v = ((double) cr_rec.timestamp)/1000.0;
#endif
  return v;
...
/*
 * Updated by Thanos Siapas 08/98. 
 *
 */
		...
		*/
		if((type = GetHeaderParameter(header,"Hostname:")) == NULL){
			...
			*/
			...
			if(strcmp(type,"duke") == 0){
				type = "i686";
			} else
			if(strcmp(type,"hawkins") == 0){
				type = "i686";
			} else {
				type = "unknown";
			}
...
char **ReadHeader(fp,headersize,nlines)
FILE	*fp;
int	*headersize;
int     *nlines;
{
...
#endif  // end of #if defined(_WINDOWS_)


int GetFileTypeTet(header)
char	**header;
{
char	*filetype;
int	type;
