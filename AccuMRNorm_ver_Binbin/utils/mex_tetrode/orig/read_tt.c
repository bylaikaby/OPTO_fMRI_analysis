...
void Disect( Parameters *parms, long *il, long *ih, double t0)
{
  ...
  while( 1 ) {
    ...
    if( timestamp < 1000*t0 ) { i1=i; } else { i2=i; }
...
void Read_Tetrode_Spike_Waveforms( Parameters *parms )
{
  ...
  for(i=0;i<parms->index.n;i++){ 
    ...
    if((parms->tstart > 0) && (tt_rec.timestamp < 1000*parms->tstart)) { continue; }
    if((parms->tend > 0) && (tt_rec.timestamp > 1000*parms->tend)) { break; }
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
  ...
  if(nrhs < 1)
    mexErrMsgTxt("LOAD_TT : FILE.TT [VERBOSE INDEX TSTART TEND]");
  if(nlhs > 1)
